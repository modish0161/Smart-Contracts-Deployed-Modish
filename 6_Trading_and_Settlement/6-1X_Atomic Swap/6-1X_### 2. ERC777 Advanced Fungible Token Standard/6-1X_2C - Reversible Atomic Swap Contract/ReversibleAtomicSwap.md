### Contract Name: `ReversibleAtomicSwap.sol`

Here’s the complete implementation for the **6-1X_2C - Reversible Atomic Swap Contract** based on your specifications.
...
contract ReversibleAtomicSwap is AccessControl, ReentrancyGuard, Pausable, IERC777Recipient {
    ...
    /**
     * @dev Initiates a new reversible atomic swap.
    ...
    /**
     * @dev Reverses the swap, returning all tokens to their original owners if conditions are not met.
     * @param _swapId The ID of the swap.
     */
    function reverseSwap(bytes32 _swapId) external nonReentrant onlyOperator(_swapId) {
        Swap storage swap = swaps[_swapId];
        ...
        IERC777(swap.initiatorToken).send(swap.initiator, swap.initiatorAmount, "");
        IERC777(swap.participantToken).send(swap.participant, swap.participantAmount, "");

        emit SwapReversed(_swapId, swap.initiator, swap.participant, swap.operator);
    }
...
2. **Contract Compilation**:
   - Save the contract code in `contracts/ReversibleAtomicSwap.sol`.
...
4. **Test Suite**:
   - Create a test file in the `test` folder, e.g., `test/ReversibleAtomicSwap.test.js`:
     ```javascript
     const { expect } = require("chai");
     const { ethers } = require("hardhat");

     describe("ReversibleAtomicSwap", function () {
       let swapContract;
       let initiator;
       let participant;
       let operator;
       ...
       beforeEach(async function () {
         ...
         const ReversibleAtomicSwap = await ethers.getContractFactory("ReversibleAtomicSwap");
         swapContract = await ReversibleAtomicSwap.deploy();
         await swapContract.deployed();

         await swapContract.addOperator(operator.address);
       });

       it("Should initiate a reversible swap with an operator", async function () {
       ...
       // Add more test cases to cover completeSwap, refundSwap, and reverseSwap functions
     });
     ```
   - Run the tests:
     ```bash
...
This setup allows for reversible atomic swaps using ERC777 tokens, providing an extra layer of security and control. If you need any additional features or customizations, feel free to ask!