### Contract Name: `OperatorControlledAtomicSwap.sol`

Here’s the complete implementation for the **6-1X_2B - Operator-Controlled Atomic Swap Contract** based on your specifications.
...
2. **Contract Compilation**:
   - Save the contract code in `contracts/OperatorControlledAtomicSwap.sol`.
...
4. **Test Suite**:
   - Create a test file in the `test` folder, e.g., `test/OperatorControlledAtomicSwap.test.js`:
     ```javascript
     const { expect } = require("chai");
     const { ethers } = require("hardhat");

     describe("OperatorControlledAtomicSwap", function () {
       let swapContract;
       let initiator;
       let participant;
       let operator;
       ...
       beforeEach(async function () {
         ...
         const OperatorControlledAtomicSwap = await ethers.getContractFactory("OperatorControlledAtomicSwap");
         swapContract = await OperatorControlledAtomicSwap.deploy();
         await swapContract.deployed();

         await swapContract.addOperator(operator.address);
       });

       it("Should initiate a swap with an operator", async function () {
...
This setup allows designated operators to manage atomic swaps on behalf of participants while maintaining control and security. If you need any additional features or customizations, feel free to ask!